...
namespace CSSynth.Sequencer
{
    public class MidiSequencer
    {
        ...
        {
            get { return (int)_MidiFile.CombinedTrack.TotalTime; }
        }
        public TimeSpan EndTime
        {
            get { return new TimeSpan(0, 0, (int)SynthHelper.getTimeFromSample(synth.SampleRate, (int)_MidiFile.CombinedTrack.TotalTime)); }
        }
        ...
        public bool LoadMidi(MidiFile midi, bool UnloadUnusedInstruments, bool autoLoadTempo = true)
        {
            if (playing == true)
                return false;
            _MidiFile = midi;
            if (_MidiFile.SequencerReady == false)
            {
                try
                {
                    ...
                    {
                        _MidiFile.CombinedTrack.MidiEvents[x].sampleTime = lastSample + (uint)DeltaTimetoSamples(_MidiFile.CombinedTrack.MidiEvents[x].sampleTime);
                        lastSample = _MidiFile.CombinedTrack.MidiEvents[x].sampleTime;
                        if (autoLoadTempo) {
                            //Update tempo
                    ...
                    _MidiFile.CombinedTrack.TotalTime = _MidiFile.CombinedTrack.MidiEvents[_MidiFile.CombinedTrack.MidiEvents.Length - 1].sampleTime;
                    //reset tempo
            ...
            if (UnloadUnusedInstruments == true)
            {
                ...
                else
                {
                    ...
                    BankManager.addBank(new InstrumentBank(synth.SampleRate, bankStr, _MidiFile.CombinedTrack.Programs, _MidiFile.CombinedTrack.DrumPrograms));
        ...
        public MidiSequencerEvent Process(int frame)
        {
            seqEvt.Events.Clear();
            //stop or loop
            if (sampleTime >= (int)_MidiFile.CombinedTrack.TotalTime)
            {
                sampleTime = 0;
                if (looping == true)
                {
            ...
            while (eventIndex < _MidiFile.CombinedTrack.EventCount && _MidiFile.CombinedTrack.MidiEvents[eventIndex].sampleTime < (sampleTime + frame))
            {
                seqEvt.Events.Add(_MidiFile.CombinedTrack.MidiEvents[eventIndex]);
                eventIndex++;
            }
            return seqEvt;
        }
        ...
        {
            return SynthHelper.getSampleFromTime(synth.SampleRate, (DeltaTime * (60.0f / (((int)_MidiFile.BeatsPerMinute) * _MidiFile.MidiHeader.deltaTicksPerQuaterNote))));
        }
        ...
        {
            while (eventIndex < _MidiFile.CombinedTrack.EventCount && _MidiFile.CombinedTrack.MidiEvents[eventIndex].sampleTime < (sampleTime + amount))
            {
                if (_MidiFile.CombinedTrack.MidiEvents[eventIndex].midiChannelEvent != MidiHelper.MidiChannelEvent.Note_On)
                    ProcessMidiEvent(_MidiFile.CombinedTrack.MidiEvents[eventIndex]);               
                eventIndex++;
            }
            sampleTime = sampleTime + amount;
        }
